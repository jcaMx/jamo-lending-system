import AppLogoIcon from '@/components/app-logo-icon';
import { BorrowersIcon } from '@/components/icons/BorrowersIcon';
import { NavFooter } from '@/components/nav-footer';
import { NavUser } from '@/components/nav-user';
import {
    Sidebar,
    SidebarContent,
    SidebarFooter,
    SidebarHeader,
} from '@/components/ui/sidebar';
import { dashboard } from '@/routes';
import { Link, usePage } from '@inertiajs/react';
import {
    Banknote,
    ChevronDown,
    ChevronRight,
    Coins,
    FileChartColumnIncreasing,
    Folder,
    HandCoins,
    LayoutGrid,
    Settings,
    User,
} from 'lucide-react';
import { useEffect, useState } from 'react';

interface BaseNavItem {
    title: string;
    icon?: React.ElementType;
    roles?: string[];
}

interface LinkNavItem extends BaseNavItem {
    type: 'link';
    href: string;
}

interface GroupNavItem extends BaseNavItem {
    type: 'group';
    subItems: LinkNavItem[];
}

type NavItem = LinkNavItem | GroupNavItem;

const mainNavItems: NavItem[] = [
    {
        type: 'link',
        title: 'Dashboard',
        icon: LayoutGrid,
        href: dashboard(),
        roles: ['admin', 'cashier'],
    },

    {
        type: 'group',
        title: 'Borrowers',
        icon: BorrowersIcon,
        roles: ['admin', 'cashier'],
        subItems: [
            { type: 'link', title: 'View All Borrowers', href: '/borrowers' },
            {
                type: 'link',
                title: 'Add Borrower',
                href: '/borrowers/add',
                roles: ['admin'],
            },
        ],
    },

    {
        type: 'group',
        title: 'Loans',
        icon: Banknote,
        roles: ['admin', 'cashier'],
        subItems: [
            { type: 'link', title: 'View All Loans', href: '/Loans/VAL' },
            { type: 'link', title: 'Past Maturity Date', href: '/Loans/PMD' },
            { type: 'link', title: '1 Month Late Loans', href: '/Loans/1MLL' },
            { type: 'link', title: '3 Month Late Loans', href: '/Loans/3MLL' },
            {
                type: 'link',
                title: 'Add Loan',
                href: '/Loans/AddLoan',
                roles: ['admin'],
            },
            {
                type: 'link',
                title: 'View Loan Applications',
                href: '/Loans/VLA',
                roles: ['admin'],
            },
        ],
    },

    {
        type: 'group',
        title: 'Repayments',
        icon: Coins,
        roles: ['cashier', 'admin'],
        subItems: [
            { type: 'link', title: 'View Repayments', href: '/repayments' },
            { type: 'link', title: 'Add Repayment', href: '/repayments/add' },
        ],
    },

    {
        type: 'group',
        title: 'Disbursements',
        icon: Coins,
        roles: ['cashier', 'admin'],
        subItems: [
            {
                type: 'link',
                title: 'Undisbursed Loans',
                href: '/disbursements/undisbursed-loans',
            },
            {
                type: 'link',
                title: 'Manage Disbursements',
                href: '/disbursements',
            },
        ],
    },

    {
        type: 'link',
        title: 'Daily Collection Sheets',
        icon: HandCoins,
        href: '/daily-collections',
        roles: ['cashier', 'admin'],
    },

    {
        type: 'group',
        title: 'Reports',
        icon: FileChartColumnIncreasing,
        roles: ['admin'],
        subItems: [
            {
                type: 'link',
                title: 'Daily Cash Position Report',
                href: '/Reports/DCPR',
            },
            {
                type: 'link',
                title: 'Monthly Report',
                href: '/Reports/MonthlyReport',
            },
        ],
    },

    {
        type: 'group',
        title: 'System Users',
        icon: User,
        roles: ['admin'],
        subItems: [
            { type: 'link', title: 'View Users', href: '/users' },
            { type: 'link', title: 'Add User', href: '/users/add' },
        ],
    },

    {
        type: 'link',
        title: 'Loan Settings',
        icon: Settings,
        href: '/Loans/loan-settings',
        roles: ['admin'],
    },
];

const footerNavItems: LinkNavItem[] = [
    {
        type: 'link',
        title: 'Repository',
        href: 'https://github.com/jcaMx/jamo-lending-system',
        icon: Folder,
    },
];

export function AppSidebar() {
    const { url, props } = usePage();
    const userRoles: string[] = props.auth?.roles ?? [];
    const [openMenus, setOpenMenus] = useState<Record<string, boolean>>({});

    const canView = (item: NavItem) => {
        if (!item.roles || item.roles.length === 0) return true;
        return item.roles.some((role) => userRoles.includes(role));
    };

    // Auto-open correct group on navigation
    useEffect(() => {
        const newOpenMenus: Record<string, boolean> = {};

        mainNavItems.forEach((item) => {
            if (!canView(item)) return;

            if (item.type === 'group') {
                const visible = item.subItems.filter(canView);
                if (visible.some((sub) => url.startsWith(sub.href))) {
                    newOpenMenus[item.title] = true;
                }
            }
        });

        setOpenMenus(newOpenMenus);
    }, [url, userRoles]);

    const toggleMenu = (title: string) => {
        setOpenMenus((prev) => ({ ...prev, [title]: !prev[title] }));
    };

    return (
        <Sidebar
            collapsible="icon"
            variant="floating"
            className="text-white w-64 flex h-full flex-col bg-[#192132]"
        >
            <SidebarHeader className="p-4 text-white bg-[#192132]">
                <Link href={dashboard()} className="gap-2 flex items-center">
                    <AppLogoIcon className="h-10 w-auto" />
                </Link>
            </SidebarHeader>

            <SidebarContent className="px-4 py-6 text-white flex-1 overflow-y-auto bg-[#192132]">
                <div className="space-y-1.5">
                    {mainNavItems.filter(canView).map((item) => {
                        const Icon = item.icon;

                        // GROUP ITEM
                        if (item.type === 'group') {
                            const visible = item.subItems.filter(canView);
                            const isOpen = openMenus[item.title] ?? false;
                            const hasActiveChild = visible.some(
                                (sub) => url === sub.href,
                            );

                            return (
                                <div key={item.title}>
                                    <button
                                        onClick={() => toggleMenu(item.title)}
                                        className={`px-4 py-3 rounded-xl flex w-full items-center justify-between transition-all duration-200 ${
                                            hasActiveChild
                                                ? 'bg-linear-to-r from-[#3c4a6a] to-[#192132]'
                                                : 'hover:bg-linear-to-r hover:from-[#2f3b57] hover:to-[#192132]'
                                        }`}
                                    >
                                        <div className="gap-3 flex items-center">
                                            {Icon && (
                                                <Icon
                                                    className={`w-5 h-5 ${hasActiveChild ? 'text-[#FABF24]' : 'text-gray-400'}`}
                                                />
                                            )}
                                            <span>{item.title}</span>
                                        </div>

                                        <span
                                            className={`transition-transform ${isOpen ? 'rotate-90' : ''}`}
                                        >
                                            {isOpen ? (
                                                <ChevronDown className="w-4 h-4" />
                                            ) : (
                                                <ChevronRight className="w-4 h-4" />
                                            )}
                                        </span>
                                    </button>

                                    {isOpen && (
                                        <div className="ml-6 mt-1.5 space-y-1.5">
                                            {visible.map((sub) => {
                                                const active = url === sub.href;

                                                return (
                                                    <Link
                                                        key={sub.title}
                                                        href={sub.href}
                                                        className={`px-4 py-2 rounded-xl text-sm relative block transition-all duration-200 ${
                                                            active
                                                                ? 'text-white bg-linear-to-r from-[#3c4a6a] to-[#192132]'
                                                                : 'hover:bg-linear-to-r hover:from-[#030916] hover:to-[#192132]'
                                                        }`}
                                                    >
                                                        {sub.title}
                                                    </Link>
                                                );
                                            })}
                                        </div>
                                    )}
                                </div>
                            );
                        }

                        // LINK ITEM
                        const isActiveParent = url === item.href;

                        return (
                            <Link
                                key={item.title}
                                href={item.href}
                                className={`gap-3 px-4 py-3 rounded-xl flex items-center transition-all duration-200 ${
                                    isActiveParent
                                        ? 'bg-linear-to-r from-[#3c4a6a] to-[#192132]'
                                        : 'hover:bg-linear-to-r hover:from-[#2f3b57] hover:to-[#192132]'
                                }`}
                            >
                                {Icon && (
                                    <Icon
                                        className={`w-5 h-5 ${isActiveParent ? 'text-[#FABF24]' : 'text-gray-400'}`}
                                    />
                                )}
                                {item.title}
                            </Link>
                        );
                    })}
                </div>
            </SidebarContent>

            <SidebarFooter className="text-white bg-[#192132]">
                <NavFooter items={footerNavItems} className="mt-auto" />
                <NavUser />
            </SidebarFooter>
        </Sidebar>
    );
}
